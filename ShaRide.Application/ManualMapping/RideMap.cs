...
namespace ShaRide.Application.ManualMapping
{
    public static class RideMap
    {
        public static ICollection<RideResponse> RidesToRideResponses(this IEnumerable<Ride> rides, IMapper mapper)
        {
            ...
            var rideResponsesTasks = rides
                .Select(ride => new Func<RideResponse>(() => MapAndGetRideReponse(mapper, ride).GetAwaiter().GetResult()))
                .ToList();

            Parallel.For(0, rideResponsesTasks.Count(), i => rideResponses.Add(rideResponsesTasks[i]()));
            return rideResponses;
        }

        private static Task<RideResponse> MapAndGetRideReponse(IMapper mapper, Ride ride)
        {
            ...
            return Task.FromResult(rideResponse);
        }
